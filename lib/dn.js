...
RDN.prototype.format = function rdnFormat(options) {
  ...
  function sortStandard(a, b) {
    ...
    if (compare === 0) {
      compare = self.attrs[a].value.localeCompare(self.attrs[b].value);
      if (compare === 0) {
        compare = Buffer.from(self.attrs[a].value).compare(Buffer.from(self.attrs[b].value));
      }
    }
    return compare;
  }
  ...
  keys.forEach((key) => {
    ...
    if (options.keepCase) {
      str += attr.name;
    } else if (options.upperName) { str += key.toUpperCase(); } else { str += key; }
...
Object.defineProperties(DN.prototype, { length: {
