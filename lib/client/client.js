...
RequestQueue.prototype.purge = function purge () {
  this.flush((msg, expect, emitter, cb) => {
...
function MessageTracker (opts) {
  ...
  const self = this;
  this.__defineGetter__('pending', () => Object.keys(self._messages));
}
...
function Client (options) {
  ...
  if (options.bindDN !== undefined &&
    ...
    this.on('setup', (clt, cb) => {
      clt.bind(options.bindDN, options.bindCredentials, (err) => {
        if (err) {
          self.emit('error', err);
        }
        cb(err);
...
Client.prototype.exop = function exop (name, value, controls, callback) {
  ...
  return this._send(req, [ errors.LDAP_SUCCESS ], null, (err, res) => {
    if (err) { return callback(err); }
...
Client.prototype.modify = function modify (name, change, controls, callback) {
  ...
  function changeFromObject (object) {
    if (!object.operation && !object.type) { throw new Error('change.operation required'); }
    if (typeof object.modification !== 'object') { throw new Error('change.modification (object) required'); }

    if (Object.keys(object.modification).length === 2 &&
        typeof object.modification.type === 'string' &&
        Array.isArray(object.modification.vals)) {
      // Use modification directly if it's already normalized:
      changes.push(new Change({
        operation: object.operation || object.type,
        modification: object.modification,
      }));
    } else {
      // Normalize the modification object
      Object.keys(object.modification).forEach((k) => {
        const mod = {};
        mod[k] = object.modification[k];
        changes.push(new Change({
          operation: object.operation || object.type,
          modification: mod,
        }));
...
Client.prototype.destroy = function destroy (err) {
  ...
  this.queue.flush((msg, expect, emitter, cb) => {
    if (typeof cb === 'function') {
      return cb(new Error('client destroyed'));
    }
    return msg;
  });
  if (this.connected) {
...
Client.prototype.connect = function connect () {
  ...
  function connectSocket (cb) {
    ...
    function onConnect () {
      ...
      self.log = self.log.child({ ldapId: tracker.id }, true);
  ...
  function setupClient (cb) {
    ...
    async.eachSeries(self.listeners('setup'), (f, callback) => {
 ...
 */
Client.prototype._onClose = function _onClose (hadError) {
  ...
  if (this.log.trace()) { this.log.trace('close event hadError=%s', hadError ? 'yes' : 'no'); }
...
Client.prototype._send = function _send (message,
  ...
  if (!this._socket || !this.connected) {
    if (!this.queue.enqueue(message, expect, emitter, callback)) {
      return callback(new ConnectionError('connection unavailable'));
    }
...
Client.prototype._sendSocket = function _sendSocket (message,
  ...
  function onRequestTimeout () {
    ...
    if (cb) {
      ...
      return cb(new errors.TimeoutError('request timeout (client interrupt)'));
    }
    return message;
  }
