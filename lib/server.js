...
function fireDTraceProbe (req, res) {
  ...
  res._dtraceOp = op;
  dtrace.fire(`server-${ op }-start`, () => probeArgs);
}
...
function Server (options) {
  ...
  function setupConnection (c) {
    ...
    c.addListener('close', (hadError) => {
      log.trace('%s close; hadError=%j', c.ldap.id, hadError);
      c.end();
    });
  ...
  function newConnection (c) {
    ...
    c.parser = new Parser({ log: options.log });
    c.parser.on('message', (req) => {
      ...
      return function next (err) {
        ...
        function after () {
          ...
          self._postChain.forEach((ch) => {
            ch.call(self, req, res, next);
          });
        }
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.add = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.bind = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.compare = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.del = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input.
 */
Server.prototype.exop = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.modify = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.modifyDN = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.search = function (name, ...args) {
 ...
 * @throws {TypeError} on bad input
 */
Server.prototype.unbind = function (...args) {
  return this._mount(Protocol.LDAP_REQ_UNBIND, 'unbind', args, true);
};

Server.prototype.use = function use (...args) {
...
Server.prototype.after = function (...args) {
  if (!this._postChain) { this._postChain = []; }

  const self = this;
  mergeFunctionArgs(args).forEach((h) => {
...
Server.prototype._sortedRouteKeys = function _sortedRouteKeys () {
  ...
  if (!this._routeKeyCache) {
    ...
    Object.keys(reversedRDNsToKeys).sort().
      reverse().
      forEach((_dn) => {
